...
encoded_data = pd.read_csv('encoded_data_with_cluster.csv')
...
""", unsafe_allow_html=True)
    ...
    if not filtered.empty:
        ...
        encoded_vector = encoded_data.drop('Cluster', axis=1).loc[idx].values.reshape(1, -1)
        cluster_label = kmeans.predict(encoded_vector)[0]

        # Get all restaurants in the same cluster
        cluster_indices = np.where(encoded_data["Cluster"] == cluster_label)[0]
